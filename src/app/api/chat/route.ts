...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // perguntas de uma palavra) vão para o modelo econômico gemini-2.5-flash â€”
    ...
    if (!activeApiKey || activeApiKey.includes("sk-or-v1-master-****")) {
      ...
      if (lowerMsg.includes("bom dia") && (periodOfDay === "Tarde" || periodOfDay === "Noite")) {
        timeCorrectionNotice = `${correctGreeting}! ðŸ˜„ Notei que você disse "bom dia", mas no relógio do sistema já são ${timeStr} (${periodOfDay.toLowerCase()}).\n\n`;
      } else if (lowerMsg.includes("boa tarde") && (periodOfDay === "Manhã" || periodOfDay === "Noite")) {
        timeCorrectionNotice = `${correctGreeting}! ðŸ˜„ Notei que você disse "boa tarde", mas no relógio do sistema já são ${timeStr} (${periodOfDay.toLowerCase()}).\n\n`;
      } else if (lowerMsg.includes("boa noite") && (periodOfDay === "Manhã" || periodOfDay === "Tarde")) {
        timeCorrectionNotice = `${correctGreeting}! ðŸ˜„ Notei que você disse "boa noite", mas no relógio do sistema já são ${timeStr} (${periodOfDay.toLowerCase()}).\n\n`;
      }
    ...
    try {
      ...
      const reqs = reqsRes || [];
      if (reqs.length > 0) {
        systemContextAddon += `- SOLICITAÃ‡Ã•ES DE COMPRA: ${reqs.length} requisições cadastradas:\n`;
        reqs.forEach((r: any) => {
    ...
    systemContextAddon += "\n[DIRETRIZES DE RESPOSTA E CONVERSAÃ‡ÃƒO MODERNAS (CRÍTICO)]\n";
    ...
    systemContextAddon += "4. NÃƒO UTILIZE NEGRITO COM ASTERISCOS EXCESSIVOS: Use parágrafos limpos e listas com 'â€¢' simples.\n";
    systemContextAddon += "5. Sempre apresente valores em R$ (padrão BRL) e percentuais com duas casas decimais.\n";
    systemContextAddon += "6. PROIBIÃ‡ÃƒO DE SAUDAÃ‡Ã•ES (CRÍTICO): Nunca inicie ou termine suas respostas com saudações sociais (Olá, Bom dia, Boa noite, Tudo bem, etc.) nem utilize emojis carinhosos (ðŸ˜Š, ðŸ˜‰, ðŸ‘). Vá imediatamente e estritamente para a resposta técnica.\n\n";

    systemContextAddon += "[COMPORTAMENTO CONVERSACIONAL E INTENÃ‡Ã•ES (OBRIGATÃ“RIO)]\n";
    systemContextAddon += "1. VOCÃŠ Ã‰ UM AGENTE CONVERSACIONAL. Seu primeiro objetivo é compreender a intenção do usuário. Nunca execute uma ação (como criar tabelas, arquivos, cadastros) apenas porque encontrou uma palavra-chave.\n";
    systemContextAddon += "2. DIFERENCIE PERGUNTAS DE COMANDOS. Se o usuário diz 'pode cadastrar cliente?', isso NÃƒO significa que ele quer cadastrar imediatamente. Ã‰ apenas uma pergunta. Você deve responder: 'Claro, posso cadastrar. Me informe Nome, CPF, Email'. Não inicie o cadastro sem ter os dados.\n";
    systemContextAddon += "3. NUNCA INVENTE OU ASSUMA DADOS (nome, email, telefone, cnpj, valor, etc.). Se faltar informação obrigatória para um comando, NÃƒO gere a saída final. Apenas PERGUNTE ao usuário o que falta.\n";
    systemContextAddon += "4. CONVERSA EM MÃšLTIPLAS ETAPAS: Você deve conseguir conduzir um cadastro naturally. Peça um dado de cada vez ou todos os faltantes, lembrando-se do estado da conversa anterior.\n";
    systemContextAddon += "5. SÃ“ EXECUTE QUANDO POSSUIR TUDO. Antes de tomar a decisão final de acionar uma ação, responda internamente: A intenção está clara? Tenho os dados obrigatórios? Se não, apenas faça perguntas de esclarecimento.\n";
