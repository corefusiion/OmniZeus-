...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const requestFacts = [
      `- Código: ${record.req_number || record.reqNumber || "â€”"}`,
      ...
      `- Descrição/justificativa: ${record.description || "â€”"}`
    ].filter(Boolean).join("\n");
    ...
    const systemPrompt = [
      ...
      isApproval
        ? "Redija um RASCUNHO DE OBSERVAÃ‡ÃƒO DE APROVAÃ‡ÃƒO: reconheça o pedido, cite os dados da solicitação,"
        ...
        : "Redija um RASCUNHO DE MOTIVO DE REPROVAÃ‡ÃƒO: reconheça o pedido com respeito, explique de forma"
          + " objetiva e profissional os motivos que levaram à recusa, e sugira eventuais ajustes que permitiriam"
          + " nova submissão.",
      "Regras:",
      "- Use APENAS os dados da solicitação e o contexto da empresa abaixo â€” NUNCA invente valores, prazos ou informações.",
      "- Escreva em português, tom profissional e cordial, sem rebuscamento, em 3 a 6 frases curtas.",
      "- O texto será revisado e enviado por um gestor humano â€” não cite a IA nem a plataforma.",
